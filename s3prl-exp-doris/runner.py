...
from torch.distributed import is_initialized, get_rank, get_world_size, is_available
...
class Runner():
    ...
    def _init_model(self, model, name, trainable, interfaces=None):
        ...
        print("unis_initialized:::::::::::::::::::::::::::::::::::::::::::",is_initialized())
        ...
        if is_initialized() and trainable and any((p.requires_grad for p in model.parameters())):
            print("DDDPPPPPPPPP###################", self.args.local_rank)
        ...
        #if(name!="Upstream"):
            #print("distribute::::::::::::::::::::::",self.args.local_rank)
            #model = DDP(model, device_ids=[self.args.local_rank], find_unused_parameters=True)
            #for interface in interfaces or []:
                #setattr(model, interface, getattr(model.module, interface))
    ...
    def _get_featurizer(self):
        model = Featurizer(
            ...
            normalize = self.args.upstream_feature_normalize,gpus = [i for i in range(torch.cuda.device_count())]
        ).to(self.args.device)
