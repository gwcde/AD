...
class Runner():
    ...
    def load_features(self, output_dir: str, sound_files: str, wavs, mode: str):
        flag = False
        for sound_file in sound_files:
            if not os.path.exists(output_dir + sound_file + ".npz"):
                flag = True

        if flag:  # 生成embeddings
            # assert 0, torch.cuda.get_device_name()

            if mode == "inference":  # inference
                wavs = [wavs.view(-1).to(self.args.device)]
            elif mode in ["train", "evaluate"]:   # train、 evaluate
            ...
            if mode == "train":
                if self.upstream.trainable:
                    ...
                    with torch.no_grad():
                        features = self.upstream.model(wavs)
            elif mode in ["evaluate", "inference"]:
                features = self.upstream.model(wavs)
            else:
                assert mode
            ...
            for name, feature in zip(sound_files, features):
                np.savez_compressed(output_dir + name, data=feature.cpu().detach().numpy())
                print(f"已保存{output_dir + name}")

        else:  # embeddings已生成
            if mode == "inference":
                features = torch.from_numpy(np.load(output_dir + x + ".npz")["data"]).cuda(self.args.device)
            else:
                features = [torch.from_numpy(np.load(output_dir + x + ".npz")["data"]).cuda(self.args.device)
                            for x in sound_files]
            print(f"已加载{sound_files}, {type(features[0])}")
        ...
        while pbar.n < pbar.total:
            ...
            for batch_id, (wavs, *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc='train', file=tqdm_file)):
                ...
                try:
                    ...
                    # 判断是否需要生成embeddings
                    # os.getcwd() --> AD/emotion_train
                    features = self.load_features(output_dir="../xlsr_53_70s/En/",
                                                  sound_files=others[1],
                                                  wavs=wavs,
                                                  mode="train")
    ...
    def evaluate(self, split=None, logger=None, global_step=0):
        ...
        if split == "dev":
            output_dir = "../xlsr_53_70s/En/"
        else:
            output_dir = "../xlsr_53_70s/Sp/"
        ...
        for batch_id, (wavs, *others) in enumerate(tqdm(dataloader, dynamic_ncols=True, desc=split, total=evaluate_steps)):
            ...
            # wavs = [torch.FloatTensor(wav).to(self.args.device) for wav in wavs]
            # 生成/加载embeddings
            features = self.load_features(output_dir=output_dir,
                                          sound_files=others[1],
                                          wavs=wavs,
                                          mode="evaluate")
            with torch.no_grad():
    ...
    def inference(self):
        ...
        with torch.no_grad():
            ...
            features = self.featurizer.model(wavs, features)
            print(type(features))
            print(type(features), str(filepath))
